import { CreateSentencesProps, SentenceControllerInterface, SentencesProps } from "@/domain/interfaces/sentence-controller-interface";
...
export class SentenceController implements SentenceControllerInterface{
    ...
    async fetchSentencesByVocabularyId(vocabularyId: string): Promise<SentencesProps[]> {
        const sentences = await prisma.sentence.findMany({
                where: {
                vocabularyId,
                },
